...
func GetMerchantsTable(ctx *context.Context) table.Table {
	...
	info.AddField("服务区域", "custom_serve_cities", db.Text).
		FieldDisplay(func(model types.FieldModel) interface{} {
			...
			for _, area := range areas {
				cities = append(cities, area.City)
			}
	...
	formList.AddField("注册资本:元", "registered_capital", db.Int8, form.Number)
	...
	formList.AddField("Registered_city_id", "registered_city_id", db.Int8, form.Text)
